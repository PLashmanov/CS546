...
import {fraudsterExists, createFraudster} from './fraudsters.js';
import {updateUserAfterReport} from './users.js';
import {updateFraudsterAfterReport} from './fraudsters.js';
...
export const removeReport = async(id) => {
    ...
    const removed = await reportsColleciton.findOneAndDelete({_id: new ObjectId(id)});

    if(!removed) throw `error: report ${id} cound not be deleted`;

...
const randomEIN = () => {
    return randomCnt(2)  + "-" + randomCnt(7)
}

const randomPhone = () => {
    //return "+" +1 + "-" +201  + "-" + randomCnt(3) + "-" + randomCnt(4)
    return "+" +1  + 201 + randomCnt(3) +randomCnt(4)
}


const randomSSNOrItin = () => {
    return randomCnt(3)  + "-" + randomCnt(2)+ "-" + randomCnt(4)
}

function randomCnt(times) {
    let res = ""
...
export function createUser(email, firstName, lastName, companyName, phoneNumber, hashedPassword) {
    return {
      ...
      firstName: ( ((firstName != null ) &&  (typeof firstName != 'number') ) ? firstName : faker.person.firstName()),
      ...
      hashedPassword: ( (hashedPassword != null )? hashedPassword : faker.internet.password()), 
      numofReports: 0,
      badge: false,
      notifications: false,
    };
  ...
  export function createFraudster(eins, itins, ssns, emails, phones, names, users, numReports,trending) {
    return {
      ...
      phones: ( (phones != null )? phones : [randomPhone(),randomPhone()]),

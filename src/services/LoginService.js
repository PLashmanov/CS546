...
import { sessionsCollection } from "../config/mongoCollections.js";
...
class LoginService {
    ...
    static async authenticate(reqEmail, reqPassword) {
        ...
        const isPasswordValid = await this.#verifyPassword(reqPassword, user.password); // Assuming your User model has a 'verifyPassword' method
        if (!isPasswordValid) {
